...
export function ModelSelector({ stage, onModelChange }: ModelSelectorProps) {
  ...
  return (
    <div ref={dropdownRef} className="relative">
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-72 bg-card rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.15)] border border-border z-50 overflow-hidden">
          {/* OpenAI Models */}
          <div className="p-2">
            <div className="px-3 py-2 text-[0.6875rem] font-semibold text-[var(--status-approved)] uppercase tracking-wider flex items-center gap-2">
              ...
              <button
                ...
                className={`w-full px-3 py-2.5 rounded-lg border-none cursor-pointer text-left transition-colors ${
                  selectedModel.id === model.id
                    ? 'bg-[color-mix(in_srgb,var(--status-approved)_10%,transparent)]'
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-foreground mb-0.5">
                      {model.name}
                      {model.isDefault && (
                        <span className="ml-2 text-[0.625rem] px-1.5 py-0.5 rounded bg-[var(--status-approved)] text-white font-semibold">
                          DEFAULT
                        </span>
                      )}
                    </div>
                  ...
                  {selectedModel.id === model.id && (
                    <svg className="w-4 h-4 text-[var(--status-approved)] shrink-0" fill="currentColor" viewBox="0 0 20 20">
          ...
          {/* Anthropic Models */}
          <div className="p-2">
            <div className="px-3 py-2 text-[0.6875rem] font-semibold text-indigo-500 uppercase tracking-wider flex items-center gap-2">
              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
            ...
            {ANTHROPIC_MODELS.map((model) => (
              <button
                ...
                className={`w-full px-3 py-2.5 rounded-lg border-none cursor-pointer text-left transition-colors ${
                  selectedModel.id === model.id
                    ? 'bg-[color-mix(in_srgb,#6366f1_10%,transparent)]'
                    : 'bg-transparent hover:bg-muted'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-foreground mb-0.5">
                      {model.name}
                      {model.isDefault && (
                        <span className="ml-2 text-[0.625rem] px-1.5 py-0.5 rounded bg-indigo-500 text-white font-semibold">
                          DEFAULT
                        </span>
                      )}
                    </div>
